"""Pure clock readings and Arabic/English speech-oriented formatting."""
...
class ClockLanguage(Enum):
	ARABIC = "ar"
	ENGLISH = "en"


class GhurubiPeriod(Enum):
	NIGHTTIME = "nighttime"
	DAYTIME = "daytime"
...
class _BaseClockFormatter:
	...
	def format_announcement(self, reading: ClockReading, clock_type: ClockType,
		...
		if options.style is AnnouncementStyle.DOUBLE:
			...
			if ghurubi_formatter.language is not self.language:
...
class NumericClockFormatter(_BaseClockFormatter):
	...
	def __new__(cls, language: ClockLanguage):
		if cls is NumericClockFormatter:
			implementation = _ArabicNumericClockFormatter if language is ClockLanguage.ARABIC else (
				_EnglishNumericClockFormatter if language is ClockLanguage.ENGLISH else None)
			if implementation is None:
				raise ValueError("unsupported clock language")
			return super().__new__(implementation)
		return super().__new__(cls)

	def __init__(self, language: ClockLanguage) -> None:
		if not isinstance(language, ClockLanguage):
			raise TypeError("language must be a ClockLanguage")
...
class _ArabicNumericClockFormatter(NumericClockFormatter, _ArabicTemplates):
	...
	def _format_zawali(self, value: datetime, options: ClockFormatOptions) -> str:
		...
		if options.hour_system is HourSystem.TWELVE:
			...
			suffix = ""
		return _numeric_civil(hour, value.minute, value.second, options, suffix, ClockLanguage.ARABIC)

	def _format_ghurubi(self, value: timedelta, options: ClockFormatOptions) -> str:
		hour, minute, second = _elapsed_parts(value)
		if options.hour_system is HourSystem.TWENTY_FOUR:
			return _numeric_elapsed(hour, minute, second, options, ClockLanguage.ARABIC)
...
class _EnglishNumericClockFormatter(NumericClockFormatter, _EnglishTemplates):
	...
	def _format_zawali(self, value: datetime, options: ClockFormatOptions) -> str:
		...
		if options.hour_system is HourSystem.TWELVE:
			...
			hour = hour % 12 or 12
		return _numeric_civil(hour, value.minute, value.second, options, suffix, ClockLanguage.ENGLISH)

	def _format_ghurubi(self, value: timedelta, options: ClockFormatOptions) -> str:
		hour, minute, second = _elapsed_parts(value)
		if options.hour_system is HourSystem.TWENTY_FOUR:
			return _numeric_elapsed(hour, minute, second, options, ClockLanguage.ENGLISH)
...
class EnglishWordClockFormatter(_EnglishTemplates):
	...
	def _format_zawali(self, value: datetime, options: ClockFormatOptions) -> str:
		...
		if options.hour_system is HourSystem.TWELVE:
			...
			result = f"{_english_number(hour)} hours"
...
def _numeric_elapsed(hour: int, minute: int, second: int, options: ClockFormatOptions,
		language: ClockLanguage) -> str:
	if minute == 0 and not options.speak_zero_minute:
		if not options.show_seconds:
			return str(hour)
		unit = _arabic_unit_phrase(second, "second", words=False) if language is ClockLanguage.ARABIC else (
			_english_unit_phrase(second, "second", words=False))
		return f"{hour} و{unit}" if language is ClockLanguage.ARABIC else f"{hour} and {unit}"
...
def _numeric_civil(hour: int, minute: int, second: int, options: ClockFormatOptions,
		suffix: str, language: ClockLanguage) -> str:
	if minute == 0 and not options.speak_zero_minute:
		if not options.show_seconds:
			return f"{hour}{suffix}"
		unit = _arabic_unit_phrase(second, "second", words=False) if language is ClockLanguage.ARABIC else (
			_english_unit_phrase(second, "second", words=False))
		return f"{hour} و{unit}{suffix}" if language is ClockLanguage.ARABIC else f"{hour} and {unit}{suffix}"
...
_ARABIC_ONES = {3: "ثلاث", 4: "أربع", 5: "خمس", 6: "ست", 7: "سبع", 8: "ثمان", 9: "تسع"}
